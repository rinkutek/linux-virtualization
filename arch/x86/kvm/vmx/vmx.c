...
const char *vm_exit_reason_names[] = {
    [0] = "Exception or NMI",
    ...
    [10] = "CPUID",
    [11] = "HLT Instruction",
    [12] = "INVLPG Instruction",
    [13] = "RDPMC Instruction",
    [14] = "RDTSC Instruction",
    [15] = "VMCALL Instruction",
    [16] = "VMCLEAR Instruction",
    [17] = "VMLAUNCH Instruction",
    [18] = "VMPTRLD Instruction",
    [19] = "VMPTRST Instruction",
    [20] = "VMREAD Instruction",
    [21] = "VMRESUME Instruction",
    [22] = "VMWRITE Instruction",
    [23] = "VMXOFF Instruction",
    [24] = "VMXON Instruction",
    [25] = "Reserved", // Reserved
    // Add more exit reasons as documented in Intel's VMX specification...
};

// Initialize the global counters structure
struct vm_exit_counters counters = {
    .total_exits = 0,
    .exit_counts = {0}, // Initialize all elements of the array to 0
};
...
int vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{
	int exit_reason = to_vmx(vcpu)->exit_reason.basic;
   	...
   	counters.total_exits++;
    	if (exit_reason < ARRAY_SIZE(counters.exit_counts)) {
        	counters.exit_counts[exit_reason]++;
    	} else {
        	printk(KERN_WARNING "Unknown exit reason: %d\n", exit_reason);
    	}

    	// Print exit statistics every 10,000 exits
    	if (counters.total_exits % 10000 == 0) {
        	printk(KERN_INFO "Exit statistics after %llu exits:\n", counters.total_exits);
        	for (int i = 0; i < ARRAY_SIZE(counters.exit_counts); i++) {
            		if (counters.exit_counts[i] > 0) {
                		const char *reason_name = (i < ARRAY_SIZE(vm_exit_reason_names) &&
                                           vm_exit_reason_names[i]) ?
                                           vm_exit_reason_names[i] : "Unknown";
                		printk(KERN_INFO "Exit Reason %d (%s): %llu exits\n",
                       			i, reason_name, counters.exit_counts[i]);
            		}
        	}
    	}
